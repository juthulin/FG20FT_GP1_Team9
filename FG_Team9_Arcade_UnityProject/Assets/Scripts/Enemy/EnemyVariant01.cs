...
public class EnemyVariant01 : MonoBehaviour
{
    ...
    private Transform _cachedPlayerTf;
    private Vector3 _targetPos;
    private Vector3 _waypoint = default;
    private Vector3 _velocity = default;
    private Vector3 _position = default;
    private Vector3 _forward = default;
    private float _fleeTimer = default;
    private float _dragAtAwake = default;
    private bool _canFire = true;
    private bool _hasAcquiredTarget = false;
    private bool _chaseTarget = false;
    private EnemyState enemyState;
    
    [SerializeField] private BaseEnemySetting enemySetting;
    [Space] 
    [SerializeField] private GameObject disabledParticleEffect;
    ...
    private void Update() //todo put waypoint into acceleration and let the only target be player, check with bool if should follow target.
    {
    ...
    private void SetWaypoint(Vector3 targetPosition)
    {
        float diff = 20f;
        float rndX = Random.Range(-diff, diff);
        float rndY = Random.Range(-diff, diff);
    ...
    private IEnumerator WaypointUpdate(float minRndCooldown, float maxRndCooldown)
    {
